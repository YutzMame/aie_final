...
def handler(event, context):
    try:
        ...
        for i, qa in enumerate(correct_answers):
            ...
            if is_flagged:
                ...
                if keywords and user_ans:
                    ...
                    is_correct = all(keyword.lower() in user_ans.lower() for keyword in keywords)
        ...
        return create_error_response(500, f"回答の処理中にエラーが発生しました: {str(e)}")
...
def create_error_response(status_code, error_message):
    return {'statusCode': status_code, 'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'}, 'body': json.dumps({"error": error_message})}